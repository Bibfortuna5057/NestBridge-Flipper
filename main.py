...
def set_temperature(value_f):
    try:
        ...
        r = urequests.post(f"{NLE_BASE}/thermostat/{DEVICE_ID}/temperature", headers=HEADERS, data=body)
        data = r.json()
        r.close()
...
def set_mode(mode):
    try:
        ...
        r = urequests.post(f"{NLE_BASE}/thermostat/{DEVICE_ID}/mode", headers=HEADERS, data=body)
        data = r.json()
        r.close()
...
def set_fan_timer(duration_seconds):
    try:
        ...
        r = urequests.post(f"{NLE_BASE}/thermostat/{DEVICE_ID}/fan", headers=HEADERS, data=body)
        data = r.json()
        r.close()
